...
async def get_false_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Пост готовий. Оберіть, що зробити:", reply_markup=reply_markup)
...
async def send_post_to_channel(context: ContextTypes.DEFAULT_TYPE, post_data):
    ...
    if post_data.get("photo"):
        await context.bot.send_photo(chat_id=channel_id, photo=post_data["photo"], caption=post_data["question"], reply_markup=reply_markup)
    else:
        await context.bot.send_message(chat_id=channel_id, text=post_data["question"], reply_markup=reply_markup)
...
async def send_post_to_user_chat(context: ContextTypes.DEFAULT_TYPE, chat_id, post_data):
    ...
    if post_data.get("photo"):
        await context.bot.send_photo(chat_id=chat_id, photo=post_data["photo"], caption=post_data["question"], reply_markup=reply_markup)
    else:
        await context.bot.send_message(chat_id=chat_id, text=post_data["question"], reply_markup=reply_markup)
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(button, pattern="^(publish_now|schedule_post|preview_post)$"))
    app.add_handler(CallbackQueryHandler(button, pattern="^(truth|false)_"))
    app.add_handler(CallbackQueryHandler(send_post_to_user_chat, pattern="^preview_"))
